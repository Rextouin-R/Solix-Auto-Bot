...
const LOGIN_URL = `${BASE_URL}/auth/login-brearer`;
...
class AccountSession {
    ...
    async checkAndClaimTasks() {
        try {
            ...
            for (const task of tasks) {
                ...
                if (task.status === 'idle') {
                    console.log(`${YELLOW}🔄 Menghitung untukk klaim task: ${task.name}${RESET}`);
                    await this.claimTask(task._id);
                } else if (task.status === 'pending') {
                    console.log(`${YELLOW}⏳ Task panding: ${task.name}${RESET}`);
                } else if (task.status === 'claimed') {
                    console.log(`${GREEN}✅ Semua task siap di klaim: ${task.name}${RESET}`);
                }
            }
            console.log(`${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}`);
    ...
    async getTasks() {
        try {
            ...
            console.error(`${RED}❌ Error task gagal untuk ${this.label}: ${error.message}${RESET}`);
            throw error;
        }
    }
    ...
    async claimTask(taskId) {
        try {
            ...
            if (response.data && response.data.result === 'success') {
                ...
                console.log(`${YELLOW}⚠️ Tidak berhasil mengklaim task: ${taskId} untuk ${this.label}${proxyInfo}${RESET}`);
                if (response.data) {
            ...
            console.error(`${RED}❌ Error mengklaim task ${taskId} untuk ${this.label}: ${error.message}${RESET}`);
        ...
        try {
            ...
            if (response.data && response.data.result === 'success' && response.data.data) {
                ...
                if (showDetailedInfo) {
                    ...
                    console.log(`${WHITE}   • 🌟 Total menambang Poin: ${pointsData.totalEarningPoint.toFixed(2)}${RESET}`);
                    ...
                    console.log(`${WHITE}⏳ ${this.label}: Memeriksa poin selanjutnya..: ${pointsData.total.toFixed(2)}${proxyInfo}${RESET}`);
                }
                
                return pointsData;
            }
            console.log(`${YELLOW}⚠️ Kesalahan memeriksa data poin untuk ${this.label}${proxyInfo}${RESET}`);
            return null;
        } catch (error) {
            console.error(`${RED}❌ Gagal memeriksa poin untuk ${this.label}: ${error.message}${RESET}`);
            if (error.response) {
                console.error(`${RED}Error details:${RESET}`, error.response.data);
            }
            return null;
    ...
    async pingConnectionQuality() {
        try {
            ...
            if (response.status === 200) {
                ...
                console.log(`${YELLOW}[${now}] ⚠️ Koneksi ping tidak merespon untuk ${this.label}${proxyInfo}: ${response.status}${RESET}`);
                return false;
            }
        } catch (error) {
            console.error(`${RED}[${new Date().toLocaleTimeString()}] ❌ Gagal menghubungkan koneksi ping untuk ${this.label}: ${error.message}${RESET}`);
    ...
    async start() {
        try {
            ...
            console.error(`${RED}❌ Masalah memulai akun ${this.label}: ${error.message}${RESET}`);
            return false;
        }
    }

    cleanup() {
        console.log(`${YELLOW}🧹 Memeriksa akun ${this.label}...${RESET}`);
        this.stopConnectionQuality();
        this.stopTaskChecker();
    }
}
...
async function loadAccounts() {
    try {
        ...
        if (fs.existsSync(ACCOUNTS_FILE)) {
            ...
            console.log(`${GREEN}✅ Berhasil masuk ${accountsData.length} akun dari ${ACCOUNTS_FILE}${RESET}`);
        } else {
            console.log(`${YELLOW}⚠️ Masalah ${ACCOUNTS_FILE} memeriksa .env untuk akun${RESET}`);

            const email = process.env.SOLIX_EMAIL;
            const brearer = process.env.SOLIX_BEARER;
            
            if (email && bearer) {
                accountsData.push({ email, bearer, label: "Default" });
                console.log(`${GREEN}✅ Default berhasil untuk .env file${RESET}`);
            }
            ...
            while (true) {
                ...
                const bearer = process.env[`SOLIX_BEARER_${accountNumber}`];
                ...
                accountsData.push({ email, bearer, label });
...
SOLIX_LABEL_2=Account Two${RESET}`);
        ...
        console.error(`${RED}❌ Error memuat akun: ${error.message}${RESET}`);
        return [];
    }
}
...
async function showAccountsStatus() {
    ...
    console.log(`${BLUE}Berhasil memeriksa ${accounts.length} akun:${RESET}`);
    
    for (let i = 0; i < accounts.length; i++) {
        const account = accounts[i];
        console.log(`${WHITE}${i + 1}. ${account.label} (${account.email})${RESET}`);
        if (account.lastPointsUpdate) {
            console.log(`   Updated selanjutnya: ${account.lastPointsUpdate.toLocaleString()}`);
        }
